...
function sln = solve_eqns(q0, dq0, x0, num_steps, parameters)
...
for i = 1:num_steps
    ...
    % used to know from which time we have to apply the external force
    if i==10
        time=sln.T{9};
        timer=time(end);
    end
    ...
    y0 = [q_p; dq_p; theta0; x0]; %re-initialization 
    t0 = T(end);
